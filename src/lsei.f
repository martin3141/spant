...
C     with units 15.-23. deleted (and nothing else changed).              
C     (Note by Yong Wang, Department of Statistics, University of Auckland,
C     New Zealand. Email: yongwang@stat.auckland.ac.nz)
C     ==============================================================

C     This package of Fortran 77 and Fortran 90 code accompanies the
C     SIAM Publications printing of "Solving Least Squares
...
C   
      RNORM=ZERO
c      GO TO (10,90,50), MODE
      IF (MODE .EQ. 2) GOTO 90
      IF (MODE .EQ. 3) GOTO 50
C                                   ********************* MODE = 1  
C                                   ALG. STEP 26
      DO 20 J=1,N  
         X(J)=G(J,NB+1)
...
C                                   ********************* MODE = 2  
 ...
 120  END DO
      RETURN
C   
cc  130 write (*,'(/a/a,4i6)')' ZERO DIAGONAL TERM IN BNDSOL.',
cc     *      ' MODE,I,J,L = ',MODE,I,J,L  
 130  Continue
cc      STOP  
      RETURN
      END
...
c     ------------------------------------------------------------------
      ...
      SAVE
c     ------------------------------------------------------------------
C      IF (ANOISE) 10,30,20  
      IF (ANOISE >= 0.0d0) GOTO 20
      MI=891
      MJ=457
      I=5   
      J=7   
...
C    ..     
 ...
 10      CONTINUE  
         IF(DIFF(HMAX+FACTOR*H(LMAX),HMAX) > 0.0d0) GOTO 50   
C   
...
C                               HERE WE ARE USING THE SOLUTION VECTOR Y.
 ...
 60   END DO         
C     
      IF (DIFF(ONE+FAC,ONE) < 0.0d0) GOTO 130 
      FAC=ONE/FAC   
      DO 90 J=1,N   
         IW=IY-1   
         DO 80 I=1,M   
...
c      subroutine MFEOUT (A, MDA, M, N, NAMES, MODE, UNIT, WIDTH)
      subroutine MFEOUT (M, N, NAMES, MODE, UNIT, WIDTH)
c
...
c
C     A(,)     Array containing matrix to be output.  Removed by Yong
C     MDA      First dimension of the array, A(,).  Removed by Yong
...
c     ------------------------------------------------------------------
      integer I, J1, J2, KBLOCK, L, LENNAM
      integer M, MAXCOL, MODE, N, NAMSIZ, NBLOCK, UNIT, WIDTH
c      double precision    A(MDA,N)
      character  NAMES(M)*(*)
      character(len=4)  HEAD (2)
      character(len=26) FMT1(2)
      character(len=26) FMT2(2)
      logical      BLKNAM, STAR
C
      data HEAD(1)/' COL'/
      data HEAD(2)/'SOLN'/
...
c
cc      write(FMT1(MODE)(6:7),'(i2.2)') NAMSIZ
cc      write(FMT2(MODE)(12:13),'(i2.2)') NAMSIZ

c 70   format(/' V-Matrix of the Singular Value Decomposition of A*D.'/
c     *     ' (Elements of V scaled up by a factor of 10**4)')
c 80   format(/' Sequence of candidate solutions, X')
...
C
      do 50 KBLOCK = 1, NBLOCK
         ...
         J2 = min(N, J2 + MAXCOL)
         if(STAR) then
cc            write (*,FMT1(MODE)) (HEAD(MODE),J,J=J1,J2)
            Continue
         else
cc            write (UNIT,FMT1(MODE)) (HEAD(MODE),J,J=J1,J2)
            Continue
         endif
C
         do 40 I=1,M
            if(STAR) then
               if(BLKNAM) then
cc                  write (*,FMT2(MODE)) I,' ',(A(I,J),J=J1,J2)
                  Continue
               else
cc                  write (*,FMT2(MODE)) I,NAMES(I),(A(I,J),J=J1,J2)
                  Continue
               endif
            else
               if(BLKNAM) then
cc                  write (UNIT,FMT2(MODE)) I,' ',(A(I,J),J=J1,J2)
                  Continue
               else
cc                  write (UNIT,FMT2(MODE)) I,NAMES(I),(A(I,J),J=J1,J2)
                  Continue
               endif
            endif
 40      continue
...
c      double precision ALPHA, ASAVE, CC, DIFF, DUMMY, FACTOR, RNORM
...
c     character*(*) NAMES(N) - this line was replaced by below due to a warning
c                              generated by GCC 16
      character(len=*), intent(in) :: NAMES(N)
      logical BLK(6), NARROW, STAR
      parameter( ZERO = 0.0d0, ONE = 1.0d0)
      parameter( TEN = 10.0d0, TWENTY = 20.0d0, THOU = 1000.0d0)
c     -- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
c  220 format (1X/' INDEX  SING. VAL.    P COEF   ',
c     *        '   RECIPROCAL    G COEF         G**2   ',
c     *        '   CUMULATIVE  SCALED SQRT'/
c     *    31x,'   SING. VAL.',26x,
c     *        '  SUM of SQRS  of CUM.S.S.')
c  221 format (1X/' INDEX  SING. VAL.    P COEF   ',
c     *        '   RECIPROCAL    G COEF     SCALED SQRT'/
c     *    31x,'   SING. VAL.',13x,'  of CUM.S.S.')
c  222 format (1X/' INDEX  SING. VAL.    G COEF         G**2   ',
c     *        '   CUMULATIVE  SCALED SQRT'/
c     *    44x,'  SUM of SQRS  of CUM.S.S.')

c  230 format (' ',4X,'0',64X,2g13.4)
c  231 format (' ',4X,'0',51X, g13.4)
c  232 format (' ',4X,'0',38X,2g13.4)

c  240 format (' ',i5,g12.4,6g13.4)

c  260 format (1X,' M  = ',I6,',   N  =',I4,',   MDATA  =',I8)
c  270 format (1X/' Singular Value Analysis of the least squares',
c     *        ' problem,  A*X = B,'/
c     *        ' scaled as (A*D)*Y = B.')
c  280 format (1X/' Scaling option No.',I2,'.  D is a diagonal',
c     * ' matrix with the following diagonal elements..'/(5X,10E12.4))
c  290 format (1X/' Scaling option No. 1.   D is the identity matrix.'/
c     * 1X)
c  300 format (1X/' INDEX',12X,'YNORM      RNORM',11X,
c     * '      LOG10      LOG10'/
c     * 45X,'      YNORM      RNORM'/1X)
c  310 format (' ',I5,6X,2E11.3,11X,2F11.3)
c  320 format (1X/
c     *' Norms of solution and residual vectors for a range of values'/
c     *' of the Levenberg-Marquardt parameter, LAMBDA.'//
c     *        '      LAMBDA      YNORM      RNORM',
c     *         '      LOG10      LOG10      LOG10'/
c     *     34X,'     LAMBDA      YNORM      RNORM')
c  330 format (1X, 3E11.3, 3F11.3)
c     ------------------------------------------------------------------
      IF (M.LE.0 .OR. N.LE.0) RETURN
      MINMN = min(M,N)
      MINMN1 = MINMN + 1
...
c                         Optionally print header and M, N, MDATA
      if(BLK(1)) then
         if(STAR) then
CC            write (*,270)
CC            write (*,260) M,N,MDATA
            Continue
         else
cc            write (UNIT,270)
cc            write (UNIT,260) M,N,MDATA
            Continue
         endif
      endif
c     -- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
...
C
 ...
 52      continue
         if(BLK(1)) then
            if(STAR) then
cc               write (*,280) ISCALE,(D(J),J = 1,N)
               Continue
            else
cc               write (UNIT,280) ISCALE,(D(J),J = 1,N)
               Continue
            endif
         endif
      endif
...
C
c      if(BLK(2)) CALL MFEOUT (A,MDA,N,N,NAMES,1, UNIT, WIDTH)
      if(BLK(2)) CALL MFEOUT (N,N,NAMES,1, UNIT, WIDTH)
c     -- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
...
cc            write (*,230) A3,A4
               ...
               if(NARROW) then
                  if(IPASS .eq. 1) then
cc               write(UNIT,231) A4
                     Continue
                  else
cc               write(UNIT,232) A3, A4
                     Continue
                  endif
               else
cc            write (UNIT,230) A3,A4
                  Continue
               endif
            endif
C
            DO 160 K = 1,MINMN
               if (SING(K).EQ.ZERO) then
                  PCOEF = ZERO
                  if(STAR) then
cc               write (*,240) K,SING(K)
                     Continue
                  else
cc               write (UNIT,240) K,SING(K)
                     Continue
                  endif
               else
                  PCOEF = B(K) / SING(K)
                  ...
                  if(STAR) then
                     if(NARROW) then
                        if(IPASS .eq. 1) then
cc                     write(*,240) K,SING(K),PCOEF,A1,B(K),      A4
                           Continue
                        else
cc                     write(*,240) K,SING(K),         B(K),A2,A3,A4
                           Continue
                        endif
                     else
cc                  write (*,240) K,SING(K),PCOEF,A1,B(K),A2,A3,A4
                        Continue
                     endif
                  else
                     if(NARROW) then
                        if(IPASS .eq. 1) then
cc                     write(UNIT,240) K,SING(K),PCOEF,A1,B(K),      A4
                           Continue
                        else
cc                     write(UNIT,240) K,SING(K),         B(K),A2,A3,A4
                           Continue
                        endif
                     else
cc                  write (UNIT,240) K,SING(K),PCOEF,A1,B(K),A2,A3,A4
                        Continue
                     endif
...
cc         write (UNIT,300)
         ...
         do 180 J = 0, NSOL
            ...
            if(STAR) then
cc            write (*,310) J,YNORM,RNORM,YL,RL
               Continue
            else
cc            write (UNIT,310) J,YNORM,RNORM,YL,RL
               Continue
            endif
 180     continue
      endif
...
C                                    COMPUTE        ALAMB = 10.0**EL
 ...
 190        CONTINUE
            ...
            IF (YNORM.GT.ZERO) YL = log10(YNORM)
            if(STAR) then
cc               write (*,330) ALAMB,YNORM,RNORM,EL,YL,RL
               Continue
            else
cc               write (UNIT,330) ALAMB,YNORM,RNORM,EL,YL,RL
               Continue
            endif
            EL = EL + DEL
 200     CONTINUE
...
C
      do 215 K = 1,NSOL
         ...
         DO 210 I = 1,N
            A(I,K) = A(I,K) * PCOEF
            IF (K.GT.1) A(I,K) = A(I,K) + A(I,K-1)
 210     END DO            
 215  continue
      if (BLK(6) .and. NSOL.GE.1)
     *     CALL MFEOUT (N,NSOL,NAMES,2,UNIT,WIDTH)
c     *     CALL MFEOUT (A,MDA,N,NSOL,NAMES,2,UNIT,WIDTH)
      return
      END
...
C                             QUIT PACKING IF FIND N NONZERO ROWS   
 ...
 60   IF (I.GT.N.OR.I.GE.M) GO TO 150   
      IF (A(I,I) .NE. 0.0d0) GOTO 90
      DO 80 J=1,N   
         IF (A(I,J) .NE. 0.0d0) GOTO 90
 80   CONTINUE  
      GO TO 100 
 90   I=I+1 
      GO TO 60  
...
C   
      if(IPASS .eq. 2) then
cc         write (*,'(/a)')
cc     *      ' FULL ACCURACY NOT ATTAINED IN BIDIAGONAL SVD'
         Continue
      endif
