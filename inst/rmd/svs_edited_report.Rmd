---
title: "spant edited SVS analysis results"
...
params:
  ...
  res_tab_ed_legacy: ""
  res_tab_ed_molal: ""
  dyn_data_uncorr_ed_off: ""
  dyn_data_corr_ed_off: ""
  dyn_data_uncorr_ed_on: ""
  dyn_data_corr_ed_on: ""
  summary_tab: ""
  plot_ppm_xlim: ""
  mri: ""
...
if (!is.null(dyn_data_uncorr_ed_off)) {
  ...
  if (!is.null(dyn_data_corr_ed_on)) {
    ...
    if (params$editing_type %in% c("gaba_1.9", "gaba_1.9_gannet")) {
      vline_f <- 3.03
    } else {
      vline_f <- 2.01
    }
    ...
    if (is.null(params$plot_ppm_xlim)) {
      image(dyn_data_corr_ed_on, xlim = c(4, 0.5), vline = vline_f,
            vline_col = "black")
    } else {
      image(dyn_data_corr_ed_on, xlim = params$plot_ppm_xlim, vline = vline_f,
            vline_col = "black")
    }
  }
  ...
  if (is.null(params$plot_ppm_xlim)) {
    image(dyn_data_uncorr_ed_on, xlim = c(4, 0.5), vline = vline_f,
          vline_col = "black")
  } else {
    image(dyn_data_uncorr_ed_on, xlim = params$plot_ppm_xlim, vline = vline_f,
          vline_col = "black")
  }
}
```
...
proc_spec_off <- params$fit_res$data
proc_spec_off <- phase(proc_spec_off, phase_offset, phi1_offset)
proc_spec_off <- shift(proc_spec_off, shift_offset, units = "ppm")
proc_spec_off <- zf(proc_spec_off)

if (is.null(params$plot_ppm_xlim)) {
  plot(proc_spec_off, xlim = c(4, 0.2))
} else {
  plot(proc_spec_off, xlim = params$plot_ppm_xlim)
}
```
...
proc_spec_on <- params$ed_on
proc_spec_on <- phase(proc_spec_on, phase_offset, phi1_offset)
proc_spec_on <- shift(proc_spec_on, shift_offset, units = "ppm")
proc_spec_on <- zf(proc_spec_on)

if (is.null(params$plot_ppm_xlim)) {
  plot(proc_spec_on, xlim = c(4, 0.2))
} else {
  plot(proc_spec_on, xlim = params$plot_ppm_xlim)
}
```
...
if (params$w_ref_available) {
  ...
  w_ref_proc <- auto_phase_bl(w_ref, ppm_start = c(5.2, 4.0),
                              ppm_end = c(5.3, 4.1), xlim = c(5.2, 4.1))
...
if (!is.null(params$fit_res_ed$res_tab$SNR)) {
  name  <- c(name, "Spectral signal to noise ratio")
  value <- c(value, round_dp(params$fit_res_ed$res_tab$SNR, 2))
}

if (!is.null(params$fit_res_ed$res_tab$SRR)) {
  name  <- c(name, "Spectral signal to residual ratio")
  value <- c(value, round_dp(params$fit_res_ed$res_tab$SRR, 2))
}

if (!is.null(params$fit_res_ed$res_tab$FWHM)) {
  name  <- c(name, "Spectral linewidth (ppm)")
  value <- c(value, round_dp(params$fit_res_ed$res_tab$FWHM, 4))
}

if (!is.null(params$fit_res_ed$res_tab$tNAA_lw)) {
  name  <- c(name, "tNAA linewidth (ppm)")
  value <- c(value, round_dp(params$fit_res_ed$res_tab$tNAA_lw, 4))
}

if (!is.null(params$fit_res_ed$res_tab$NAA_lw)) {
  name  <- c(name, "NAA linewidth (ppm)")
  value <- c(value, round_dp(params$fit_res_ed$res_tab$NAA_lw, 4))
}

if (!is.null(params$fit_res_ed$res_tab$tCho_lw)) {
  name  <- c(name, "tCho linewidth (ppm)")
  value <- c(value, round_dp(params$fit_res_ed$res_tab$tCho_lw, 4))
}

if (!is.null(params$fit_res_ed$res_tab$Cho_lw)) {
  name  <- c(name, "Cho linewidth (ppm)")
  value <- c(value, round_dp(params$fit_res_ed$res_tab$Cho_lw, 4))
}

if (!is.null(params$fit_res_ed$res_tab$tCr_lw)) {
  name  <- c(name, "tCr linewidth (ppm)")
  value <- c(value, round_dp(params$fit_res_ed$res_tab$tCr_lw, 4))
}

if (!is.null(params$fit_res_ed$res_tab$Cr_lw)) {
  name  <- c(name, "Cr linewidth (ppm)")
  value <- c(value, round_dp(params$fit_res_ed$res_tab$Cr_lw, 4))
}

if (!is.null(params$fit_res_ed$res_tab$phase)) {
  name  <- c(name, "Zero-order phase (degrees)")
  value <- c(value, round_dp(params$fit_res_ed$res_tab$phase, 1))
}

if (!is.null(params$fit_res_ed$res_tab$phi1)) {
  name  <- c(name, "First-order phase (ms)")
  value <- c(value, round_dp(params$fit_res_ed$res_tab$phi1, 3))
}

if (!is.null(params$fit_res_ed$res_tab$shift)) {
  name  <- c(name, "Frequency offset (ppm)")
  value <- c(value, round_dp(params$fit_res_ed$res_tab$shift, 4))
}
...
if (params$fit_res_ed$method == "ABFIT") {
  name  <- c(name, "Fit quality number (FQN)",
                   "Baseline effective d.f. per ppm",
                   "Lineshape asymmetry")
  value <- c(value, round_dp(params$fit_res_ed$res_tab$FQN, 2),
             round_dp(params$fit_res_ed$res_tab$bl_ed_pppm, 2),
             round_dp(params$fit_res_ed$res_tab$asym, 2))
}
