...
stackplot(params$fit_res_ed, y_offset = 0.5, combine_lipmm = TRUE, labels = TRUE,
...
if (!is.null(dyn_data_uncorr_ed_off)) {
  ...
  if (!is.null(dyn_data_corr_ed_off)) {
    ...
    if (is.null(params$plot_ppm_xlim)) {
      image(dyn_data_corr_ed_off, xlim = c(4, 0.5))
    } else {
      image(dyn_data_corr_ed_off, xlim = params$plot_ppm_xlim)
    }
  }
  if (!is.null(dyn_data_corr_ed_on)) {
    cat("\n\n### Edit-on with dyn. correction\n")
    if (is.null(params$plot_ppm_xlim)) {
      image(dyn_data_corr_ed_on, xlim = c(4, 0.5))
    } else {
      image(dyn_data_corr_ed_on, xlim = params$plot_ppm_xlim)
    }
  }
  cat("\n\n### Edit-off without dyn. correction\n")
  if (is.null(params$plot_ppm_xlim)) {
    image(dyn_data_uncorr_ed_off, xlim = c(4, 0.5))
  } else {
    image(dyn_data_uncorr_ed_off, xlim = params$plot_ppm_xlim)
  }
  cat("\n\n### Edit-on without dyn. correction\n")
  if (is.null(params$plot_ppm_xlim)) {
    image(dyn_data_uncorr_ed_on, xlim = c(4, 0.5))
  } else {
    image(dyn_data_uncorr_ed_on, xlim = params$plot_ppm_xlim)
  }
}
```
